...
class _ReviewListScreenState extends State<ReviewListScreen> {
  ...
  void initState() {
    ...
    localCountryFilter = widget.mode == 'requested' ? 'ALL' : SessionCache.countryFilter;
    ...
    if (userId != null) {
      ...
      _reviewSubscription = _reviewsRef.onValue.listen((event) {
        ...
        final reviews = data.entries
            .where((e) => !e.key.toString().startsWith('_')) // Exclude meta records like _meta
            .map((e) {
  ...
  void _applyFiltersAndSort() {
    ...
    final filtered = _allReviews.where((review) {
      ...
      final friendMatch = friend == null || friend == 'ALL' || (review['userEmail']?.toString() ?? '') == friend;
  ...
  final String previewMode = widget.mode == 'requested' ? 'requested' : 'preview';
  Navigator.push(context, MaterialPageRoute(builder: (_) => PreviewScreen(context: reviewContext, mode: previewMode)));
  }
  ...
  Future<void> _handleDeleteRequestedReviews() async {
    if (_filteredReviews.isEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No reviews to delete')),
      );
      return;
    }
    ...
    final bool? confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete these ${_filteredReviews.length} review(s)?'),
        actions: [
          TextButton(
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error deleting reviews: $e')),
      );
    }
  }
