...
class FriendRow extends StatelessWidget {
  ...
  Future<void> _resolveRvCountForEntry() async {
    try {
      ...
      if (entry.reviewRequest != null) {
        patch['users/$myUidNow/friends/${entry.uid}/review_request/rvCount'] =
            found;
        patch['users/$myUidNow/friends/${entry.uid}/review_request/updatedAt'] =
            nowIso;
        patch['users/$myUidNow/friends/${entry.uid}/rvCount'] = found;
        patch['users/$myUidNow/friends/${entry.uid}/rvCountLastCheckedAt'] =
            nowIso;
      } else {
  ...
  String _statusTokenWithCounts() {
    ...
    final int exCount =
        entry.reviewRequest?.exCount ?? entry.review?.exCount ?? 0;

    if (entry.status == FriendStatus.rvWants ||
  ...
  Widget build(BuildContext context) {
    ...
    comment = (rawComment != null && (!hideComment || showReviewComment))
        ? rawComment
        : null;
                  ...
                  children: <Widget>[
                    Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: statusColor.withAlpha(borderAlpha),
                        ),
                      ),
                      child: Text(
                        statusToken,
                        style: AppFonts.small.copyWith(color: statusColor),
                      ),
                    ),
                    const SizedBox(height: 6),
                    if (entry.sharedReviewsCount > 0)
                      Text(
                        '+${entry.sharedReviewsCount}',
                        style: AppFonts.smallHint.copyWith(
                          color: AppColors.mutedText,
                        ),
                      ),
